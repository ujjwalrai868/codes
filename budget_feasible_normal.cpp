...
map<int,int> bid_mapper(vector<int>v,int qvn,int curr_budget_value)
{
  ...
  cout<<"Bidder  Bid\n";
  normal_distribution<double> func(curr_budget_value/2,curr_budget_value/4);
	for(i=0;i<qvn;i++)
	{
		   int ran=int(func(e));
       if(ran>0)
...
int main()
{
   ...
   int proposed_budget_value,utility_sum,average_payment;
 ...
 for(value=0;value<100;value++)
 {
  ...
  //for each task we will have a winning set
    ...
    //calculate utility also
    //utility=payment-valuation
      for(i=0;i<key;i++)
      {
        if(i==0&&value==0&&j==0)
        {
          no_use_value=utility_sum;
        }
        cout<<"previous utility_sum is: "<<utility_sum<<endl;
        cout<<"win_set and payment are: "<<win_set[i]<<" "<<payment<<endl;
        utility_sum=utility_sum+(payment-win_set[i]);
        cout<<"current utility_sum is: "<<utility_sum<<endl;
      }
    }
 ...
 //for calculating average payment 
   cout<<"all total_payments were: "<<endl;
   for(i=0;i<100;i++)
    cout<<total_payment[i]<<" ";
   for(i=0;i<100;i++)
    average_payment=average_payment+total_payment[i];
   cout<<"Average payment is :"<<average_payment/100<<endl;
   cout<<"average utility is: "<<utility_sum/100<<endl;
    return 0;
}