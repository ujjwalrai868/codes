...
void dfs_traversal(int x,int visited[],int start_time[])
{
   ...
   for(i=0;i<G[x-1].size();i++)
   {
   	if(!visited[G[x-1][i]-1])
    {
      ...
      {
        if(visited[G[x-1][j]])
          check_if_all_visited=true;
        else
        {
          check_if_all_visited=false;
          break;
        }
      }
      if(!check_if_all_visited);
      {
          v[x-1].push_back(G[x-1][i]);
          child.insert(pair<int,vector<int>>(x,v[x-1]));
      }
...
int main()
{
  ...
  int visited[G.size()]={0};
    for(i=0;i<G.size();i++)
    {
       ...
       for(j=0;j<G.size();j++)
        visited[j]=0;
       dfs_traversal(i+1,visited,start_time);
       dfs_all.push_back(dfs);  
    }
    for(i=1;i<=G.size();i++)
     {
      dfs.clear();
    ...
    cout<<"dfs_all is: "<<endl;
    for(i=0;i<dfs_all.size();i++)
    {
      for(j=0;j<dfs_all[i].size();j++)
        cout<<dfs_all[i][j]<<" ";
      cout<<endl;
    }
    cout<<"number of cycles are: "<<cycle_count<<endl;
    cout<<"child map is: "<<endl;
    for(ptr=child.begin();ptr!=child.end();ptr++)
    {
