...
int main()
{
   int n,ni,N,qvn,i,j,k,ran,budget_value,t,sum,first_loser,total_time,curr_budget_value,in;
   //ni is the number of installments
   int total_payment=0;
   int proposed_budget_value;
   ni=5;
   int installments[ni];
 ...
 while(qvn>0)
 {
  ...
  for(j=0;j<N;j++)
  {
    ...
    for(itr=win_map.begin();itr!=win_map.end();itr++)
    {
      total_payment=total_payment+itr->second;
    }
  }
  cout<<"Proposed budget_value was :"<<proposed_budget_value<<endl;
 // cout<<"Rs "<<(proposed_budget_value*3)/10<<" is given to each peer graders"<<endl;
  cout<<"Total money spend for the task is : "<<total_payment<<endl;
  return 0;
}
