...
no_of_types=input("Enter number of types of resources:")
for i in range(no_of_types):
	...
	for j in range(inpt):
		...
		if no_res>r.capacity():
			print 'sorry,demand is greater than capacity'
			continue
		s=r.make_reservaton()
		if s=='reject' and len(slots)>1:
			...
			count_dash+=1
		if s=='reject' and len(slots)==1 and len(slots)==req_time:
		...
		for w in range(len(div_slots)):
			if len(div_slots[w])>=req_time:
				...
				print 'making reservation again'
